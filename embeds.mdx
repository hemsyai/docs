...
function HemsyEmbedSetup() {
  const { linesAdd, linesRemove, lines } = useCart();
...
Also update the "Script Attributes Reference" table to mention callbacks:
...
Note: For event callbacks (`onItemAdded`, `onItemRemoved`), use `window.HemsyEmbedConfig`
instead of data attributes. See the Event Callbacks section below.
